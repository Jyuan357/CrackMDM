...
class DecoderBottleneck(nn.Module):
    ...
    def forward(self, x, x_concat=None):
        # print('decoderbottleneck x {}'.format(x.shape) )
        x = self.upsample(x)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, x1, x2, x3):
        # print("decoder ", x.shape, x1.shape, x2.shape, x3.shape)
...
class TransUNet(nn.Module):
    def __init__(self, img_dim=448, in_channels=3, out_channels=128, head_num=4, mlp_dim=512, block_num=8, patch_dim=16, class_num=1):
        super().__init__()
...
if __name__ == '__main__':
    ...
    # print(transunet(torch.randn(1, 3, 128, 128).cuda()).shape)
