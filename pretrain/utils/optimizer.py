...
def build_finetune_optimizer(config, model, logger):
    ...
    if config.MODEL.TYPE == 'swin':
        ...
        num_layers = 17
        get_layer_func = partial(get_fouriervit_layer, num_layers=num_layers + 1)
    else:
...
def get_fouriervit_layer(name, num_layers):
    if name.startswith("down"):
        layer_id = int(name.split('.')[0][-1]) - 1
    elif name.startswith("up"):
        layer_id = 4 - int(name.split('.')[0][-1]) + 4 
    elif name.startswith("fuse"):
        layer_id = 5 - int(name.split('.')[0][-1]) + 8
    elif name.startswith("serial"):
        layer_id = 4 - int(name.split('.')[0][-1]) + 13 
    else:
