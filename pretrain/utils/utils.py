...
def load_checkpoint(config, model, optimizer, lr_scheduler, logger):
    ...
    if not config.EVAL_MODE and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
        ...
        if 'max_ods' in checkpoint:
            max_ods = checkpoint['max_ods']
        if 'max_ois' in checkpoint:
            max_ois = checkpoint['max_ois']
        epoch = checkpoint['epoch']

    del checkpoint
    torch.cuda.empty_cache()
    return max_accuracy, max_ods, max_ois, epoch


def save_checkpoint(config, epoch, model, max_accuracy, max_ods, max_ois, optimizer, lr_scheduler, logger):
...
def load_pretrained(config, model, logger):
    ...
    elif config.MODEL.TYPE == 'FourierVit':
        logger.info(f">>>>>>>>>> Remapping pre-trained keys for FourierVit ..........")
        model_dict = model.state_dict()
        droplist = ['head.weight', 'head.bias', 'aux_head.weight', 'aux_head.bias']
        checkpoint = {k: v for k, v in checkpoint_model.items() if k in model_dict and k.startswith('down')}

    else:
        # raise NotImplementedError  
        logger.info(f">>>>>>>>>> Not remapping pre-trained keys ..........") 
