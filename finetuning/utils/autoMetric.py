...
class CollectData:
	...
	def statistics(self):
		...
		for threshold in tqdm(range(0,256)):
			temp_TP=0.0
			temp_FP=0.0
			temp_FN=0.0
			temp_TN=0.0
			...
			for index in range(len(self.groundtruth)):
				...
				gt_img=self.groundtruth[index]
				prob_img=self.probgraph[index]
			...
			p_temp = (temp_TP) / (temp_TP + temp_FP  + 1e-4)
			r_temp = temp_TP / (temp_TP + temp_FN + 1e-4)
			if p_temp*r_temp > P*R:
				P = p_temp
				R = r_temp
		...
		FPR = (self.FP) / (self.FP + self.TN + 1e-4)
		TPR = (self.TP) / (self.TP + self.FN + 1e-4)
		AUC = np.round(np.sum((TPR[1:] + TPR[:-1]) * (FPR[:-1] - FPR[1:])) / 2., 4)

		Precision = (self.TP) / (self.TP + self.FP  + 1e-4)
		Recall = self.TP / (self.TP + self.FN + 1e-4)
		MAP = np.round(np.sum((Precision[1:] + Precision[:-1]) * (Recall[:-1] - Recall[1:])) / 2.,4)
		F1 = np.max(2 * (Precision * Recall) / (Precision + Recall  + 1e-4))

		iou = self.IoU()
		dice = self.dice()

		return (FPR,TPR,AUC),(Precision,Recall,MAP), F1, iou, dice
	
	def dice(self, threshold=128):
		'''
		...
		for index in range(len(self.groundtruth)):
			...
			intersection = intersection + np.sum(gt_img*prob_img) * 2
			union = union+np.sum(gt_img) + np.sum(prob_img)
		
		dice=np.round(intersection/union, 4)
...
def drawCurve(gtlist,problist,modelName,dataset,savepath='./'):
	...
	for index in range(modelNum):
		...
		(FPR, TPR, AUC), (Precision, Recall, MAP), _, IoU, DICE = process.statistics()
